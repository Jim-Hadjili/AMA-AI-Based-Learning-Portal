...
if ($quizSubmissions && $quizSubmissions instanceof mysqli_result && $quizSubmissions->num_rows > 0) {
    ...
    while ($row = $quizSubmissions->fetch_assoc()) {
        ...
        // Get all quiz IDs in the chain (original + AI-generated descendants)
        ...
        if ($latestAttemptRow = $latestAttemptRes->fetch_assoc()) {
            ...
            $latestScoreDisplay = "{$correctAnswers}/{$totalQuestions}";
            $latestScorePercent = $totalQuestions > 0 ? round(($correctAnswers / $totalQuestions) * 100) : 0;
        }
        ...
        if (isset($classNames[$row['class_id']])) {
            $activities[] = [
                ...
                'latest_score' => $latestScoreDisplay ?? "{$latestScore}/?", // fallback
                'latest_score_percent' => $latestScorePercent ?? 0,
