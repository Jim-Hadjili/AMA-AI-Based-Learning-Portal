...
<body class="bg-gray-50 font-[Poppins]">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Prepare data for the chart - we need to sort attempts by chronological order for the chart
            const attemptData = <?php 
                // Create an array with attempt numbers and scores, keeping them paired
                $chartData = array_map(function($attemptItem) use ($conn, $attempt_id) {
                    ...
                    return [
                        'number' => $attemptItem['attempt_number'],
                ...
                // Sort by attempt number for the chart
            ...
            // Extract labels and scores in the correct order
            const labels = attemptData.map(item => "Attempt " + item.number);
            const scores = attemptData.map(item => item.score);
            
            // Create background colors - AI-generated ones get a different color
            const backgroundColors = attemptData.map(item => 
                item.isAI ? 'rgba(79, 70, 229, 0.5)' : 'rgba(59, 130, 246, 0.5)'
            );
            
            const borderColors = attemptData.map(item => 
                item.isAI ? 'rgb(67, 56, 202)' : 'rgb(59, 130, 246)'
            );
                        ...
                        tooltip: {
                            callbacks: {
                                label: function(context) {
                                    const item = attemptData[context.dataIndex];
